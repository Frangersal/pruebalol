...
$query_Recordset1 = "SELECT * FROM bloques";
...
$numerobloque = $row_Recordset1['numerobloque'];

$query_Recordset2 = "SELECT * FROM lecciones ";
$Recordset2 = mysql_query($query_Recordset2, $conamatenlinea) or die(mysql_error());
$row_Recordset2 = mysql_fetch_assoc($Recordset2);
$totalRows_Recordset2 = mysql_num_rows($Recordset2);

$id = $row_Recordset2['id'];
$idbloque = $row_Recordset2['idbloque'];
$cuerpo = $row_Recordset2['cuerpo'];

if ($_POST) {

    $idmateria = addslashes(addslashes(trim($_POST['idmateria'])));
    $nombrebloque = addslashes(addslashes(trim($_POST['nombrebloque'])));
    $numerobloque = addslashes(addslashes(trim($_POST['numerobloque'])));

    $id = addslashes(addslashes(trim($_POST['id'])));
    $idbloque = addslashes(addslashes(trim($_POST['idbloque'])));
    $cuerpo = addslashes(addslashes(trim($_POST['cuerpo'])));
    
    mysql_query("INSERT INTO bloques( idmateria, nombrebloque, numerobloque) VALUES('$idmateria', '$nombrebloque', '$numerobloque')"); 
    
    mysql_query("INSERT INTO lecciones( id, idbloque, cuerpo) VALUES( '$id', '$idbloque', '$cuerpo')"); 
            ...
            <div class="opcionactual" style=" width: calc(100% - 4px); float: left; background-color: #2a3444; border-left: 4px solid #4a8ab1;">
                ...
                <div id="seccionprinpal" style="width:80%; margin-left: 20%; float: left; background: white; ">
                    ...
                    <div id="contenedor" style="width: calc(100% - 10px);float:left; padding: 5px;position:relative;">
                        ...
                        <div id="titulo" style="text-align: center; font-size: 30px; font-weight: bold;margin: 20px 0px; padding: 5px; color: #187ba8; width: calc(100% - 10px);">BLOQUES</div>
                        ...
                        <div id="toprow" style="width: calc(100% - 10px); float:left; padding: 5px;">
                                ...
                                <input type="text" id="nombrebloque" name="nombrebloque" value="" placeholder="Nombre del bloque" style="padding: 10px;  width: calc(30% - 32px); float:left; margin: 5px;background: #afdaff;">

                                <div id="bloqued1" style="background: aliceblue;padding: 10px; font-size: 10px; width: calc(70% - 32px); float:left; margin: 5px;">

                                    <textarea id="cuerpo" name="cuerpo" placeholder="Nombre y contenido de la lección" value="" style="width: calc(100% - 6px); resize: none; float: left; margin: 3px;"></textarea>
                ...
                <div id="contenedorbotones" align="center" style="float: left; width: 100%; margin-top: 50px; margin-bottom: 20px;">
                    ...
                    <button type="submit" onclick="window.location='bloques.php'" style="border: solid #595f61 2px; background: #094866c9; border-radius: 5px; width: 100px; font-size: 12px; cursor: pointer; color: white; padding: 10px 0px;">GUARDAR</button>
