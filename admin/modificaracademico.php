...
if ($_POST) {
    ...
    $biografia = addslashes(addslashes(trim($_POST["biografia"])));
    $idacademico = addslashes(addslashes(trim($_POST["id"])));

mysql_select_db($database_conamatenlinea, $conamatenlinea);
$query_Recordset3 = "SELECT * FROM academicos WHERE id = '$idacademico'";
...
function guardarform(formnumber) {
    ...
    if (typeof $('#foto' + formnumber)[0].files.length !== 0) {
    ...
    form_data.append('id' , $("#id" + formnumber).val());
...
?>
    ...
    <div id="contenedor-academico<?php echo $id; ?>" class="card" align="center" style="width:calc(70% - 120px); margin-left: 15%; float: left; margin-bottom: 40px;">
        
        <form id="academicoform<?php echo $id; ?>" name="academicoform<?php echo $id; ?>" action="" enctype="multipart/form-data" method="post">

            <div id="titulo" style="width: 100%; margin-bottom: 40px; text-align: center; font-size: 25px; color: #58575d; font-weight: bold; letter-spacing: 1.6;">Modificar académico</div>

            <input id="id<?php echo $id; ?>" name="id<?php echo $id; ?>" value="<?php echo $id; ?>" type="hidden">

            <div id="contenedorsubidaimagen" style="float: left; width: calc(30% - 20px); margin-right: 20px;">

                <div id="contenedorimagenactual<?php echo $id; ?>" class="imagenmaestro" align="center" style="background-image: url(../images/<?php echo $imagen; ?>);"></div>

                 <a href="javascript:void(0);" onclick="javascript:document.getElementById('foto<?php echo $id; ?>').click();"><div id="botonfile" align="center" style="border: 1px solid #1d4267; background-color: #1d4267; margin-top: 20px; color: #7FDBFF; padding:10px; cursor:pointer; width: calc(100% - 22px); float: left;">
                    ...
                    <input type="file" id="foto<?php echo $id; ?>" name="foto<?php echo $id; ?>" onchange="javascript:enviar('<?php echo $id; ?>');" style="display: none;" accept="image/jpeg" />  
            ...
            <input id="academiconombre<?php echo $id; ?>" name="academiconombre<?php echo $id; ?>" placeholder="Escribe un nombre" style="resize: none; background-color: #0b2c4d; font-size: 12px; background-color: white; width: calc(70% - 12px); margin-bottom: 10px; padding: 5px; float: left; border: 1px solid lightgray;" type="text" value="<?php echo $nombre; ?>">

            <textarea id="academicobiografia<?php echo $id; ?>" name="academicobiografia<?php echo $id; ?>" placeholder="Escribe una biografía" style="resize: none; background-color: #0b2c4d; height: 170px; font-size: 12px; background-color: white; width: calc(70% - 12px); padding: 5px; float: left; border: 1px solid lightgray;"><?php echo $biografia; ?></textarea>

            <div id="contenedorboton" align="center" style="width: 100%; margin-top: 60px; float: left; display: flex; align-items: center; justify-content: center;">

                <div id="contiene" style="float: left;">
            
                    <button onclick="guardarform('<?php echo $id; ?>');" style="border-radius: 10px; line-height: 30px; background-color: #1d4267; border-color: #7FDBFF; padding-left: 20px; padding-right: 20px;color: #7FDBFF; cursor: pointer; float: left; margin-right: 40px;">Guardar cambios</button>
                
                    <input type="button" name="Cancelar" value="Cancelar" onclick="window.location='academicos.php'" style="float: left; width: 100px; margin-bottom: 20px; padding: 10px 0px; background-color: #00020287; border-color: #b8bfc1; color: #d8dfe2; cursor: pointer; border-radius: 10px;">
