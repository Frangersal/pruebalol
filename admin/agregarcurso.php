...
$query_Recordset14 = "SELECT * FROM materias";
$Recordset14 = mysql_query($query_Recordset14, $conamatenlinea) or die(mysql_error());
$row_Recordset14 = mysql_fetch_assoc($Recordset14);
$totalRows_Recordset14 = mysql_num_rows($Recordset14);

$query_Recordset14a = "SELECT * FROM materias";
$Recordset14a = mysql_query($query_Recordset14a, $conamatenlinea) or die(mysql_error());
$row_Recordset14a = mysql_fetch_assoc($Recordset14a);
$totalRows_Recordset14a = mysql_num_rows($Recordset14a);

$query_Recordset13 = "SELECT * FROM maestros";
$Recordset13 = mysql_query($query_Recordset13, $conamatenlinea) or die(mysql_error());
$row_Recordset13 = mysql_fetch_assoc($Recordset13);
$totalRows_Recordset13 = mysql_num_rows($Recordset13);

$query_Recordset13a = "SELECT * FROM maestros";
$Recordset13a = mysql_query($query_Recordset13a, $conamatenlinea) or die(mysql_error());
$row_Recordset13a = mysql_fetch_assoc($Recordset13a);
$totalRows_Recordset13a = mysql_num_rows($Recordset13a);

$query_Recordset15 = "SELECT * FROM cursos";
$Recordset15 = mysql_query($query_Recordset15, $conamatenlinea) or die(mysql_error());
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            function search() {
                ...
                $.post("buscarmaterias.php?numSelec=" + num + "", {
                    userinput: "" + $('#materia').val() + ""
                }, function(data) {
                    if (data.length > 0) {
            ...
            function search2() {
                ...
                $.post("buscarlibros.php?numSelec=" + num + "", {
                    userinput: "" + $('#libro').val() + ""
                }, function(data) {
                    if (data.length > 0) {
        ...
        function guardarform(formnumber) {
            ...
            form_data.append('idcurso', $("#idcurso" + formnumber).val());
       ...
       function agregarcurso(){
            ...
            //Paso 1 consulta
            var nmaterias = parseInt($("#cmaterias").val());

            //Paso 2 actualizacion

            var num2 = parseInt(nmaterias,10) + parseInt(1,10);

            //Guardo el valor de mivariable (para que persista el valor despues de la ejecucion de la funcion)
            $("#cmaterias").val(num2);

            //Paso 3 creo el elemento dinamico
            var plantilla = $('#contenedorselects');
          
         var contenido = '<div id="contentido" style="width:100%; float:left; margin-top:10px;"> Curso '+num2+'</div> <select id="materias"name="materias" style="font-size: 12px;width: calc(50% - 20px);margin: 10px 0;box-sizing: border-box;padding: 10px 10px;border: 1px solid #e8e1e1;background-color: whitesmoke;color: gray;float: left;">Materias<option value="1">Agregar materias</option><?php if($totalRows_Recordset14a > 0) do{$idmateria = $row_Recordset14a["id"]; $materia = $row_Recordset14a["nombre"];?><option value="<?php echo $idmateria; ?>'+num2 +'"><?php echo $materia; ?></option><?php }while($row_Recordset14a = mysql_fetch_assoc($Recordset14a));?></select><div id="lineaunion" style="width: 30px;border:solid lightgray;height: 0px;float: left;margin-top: 26px;"></div><select id="maestros " name="maestros"style="font-size: 12px;width: calc(50% - 15px);margin: 10px 0;box-sizing: border-box;padding: 10px 10px;border: 1px solid #e8e1e1;background-color: whitesmoke;color: gray;float: right;">Maestros<option value="1">Agregar maestro</option><?php if($totalRows_Recordset13a > 0)do{$idmaestro = $row_Recordset13a["id"];$maestro = $row_Recordset13a["nombre"];?><option value="<?php echo $idmaestro; ?>"><?php echo $maestro; ?></option><?php
    } while ($row_Recordset13a = mysql_fetch_assoc($Recordset13a)); ?></select>';


    plantilla.append(contenido);
        ...
        function eliminarcurso(){
            ...
            //Paso 1
            nmaterias = parseInt($("#cmaterias").val());
          
            //Paso 2
            if(nmaterias == 2)
            {
                $("#botoenliminar").css('display','block');
            }
          ...
          var contenido = '<div id="contentido" style="width:100%; float:left; margin-top:10px;"> Curso '+num2+'</div> <select id="materias"name="materias" style="font-size: 12px;width: calc(50% - 20px);margin: 10px 0;box-sizing: border-box;padding: 10px 10px;border: 1px solid #e8e1e1;background-color: whitesmoke;color: gray;float: left;">Materias<option value="1">Agregar materias</option><?php if($totalRows_Recordset14a > 0) do{$idmateria = $row_Recordset14a["id"]; $materia = $row_Recordset14a["nombre"];?><option value="<?php echo $idmateria; ?>"><?php echo $materia; ?></option><?php }while($row_Recordset14a = mysql_fetch_assoc($Recordset14a));?></select><div id="lineaunion" style="width: 30px;border:solid lightgray;height: 0px;float: left;margin-top: 26px;"></div><select id="maestros " name="maestros"style="font-size: 12px;width: calc(50% - 15px);margin: 10px 0;box-sizing: border-box;padding: 10px 10px;border: 1px solid #e8e1e1;background-color: whitesmoke;color: gray;float: right;">Maestros<option value="1">Agregar maestro</option><?php if($totalRows_Recordset13a > 0)do{$idmaestro = $row_Recordset13a["id"];$maestro = $row_Recordset13a["nombre"];?><option value="<?php echo $idmaestro; ?>"><?php echo $maestro; ?></option><?php
     } while ($row_Recordset13a = mysql_fetch_assoc($Recordset13a)); ?></select>';
            
            $('#cmaterias'+num2).remove(this.contenido);
...
<body>
    ...
    <div id="wrapper" style="width: 100%; float: left; height: 100vh; background-color: #0b2c4d; position: relative;">
        ...
        <div id="seccionprincipal" style="height: 100vh; float: left; width: 80%; position: relative; margin-left: 20%;">
            ...
            <a href="Javascript:void(0)" onclick="nuevo()">
                <div id="botonagregar" align="center" style=" width: 100%; float: left; background: #305b84; padding-top: 20px; padding-bottom: 20px; cursor: pointer; font-size: 16px; font-family: 'Montserrat', sans-serif; text-decoration: underline;">Agregar curso</div>
            </a>

            <div id="contenedor-curso0" class="card" style="width: calc(100% - 60px); float: left; margin-bottom: 40px;">

                <div id="titulocurso" align="center" style="font-size: 25px; line-height: 60px; float: left; width: 100%;">Agregar curso</div>
                ...
                <form id="form1" name="form1" action="" enctype="multipart/form-data" method="post">
                    ...
                    <div id="contienesubidaimagen" style="width: 25%; float: left;">
                        ...
                        <a href="javascript:void(0);" onclick="javascript:document.getElementById('foto0').click();">
                            <div id="botonfile" align="center" style="border: 1px solid #1d4267; background-color: #1d4267; color: #7FDBFF; padding: 5px; cursor:pointer; margin-top: 10px; width: calc(100% - 12px); float: left;">
                        ...
                        <div id="up0" style="width:calc(100% - 22px); font-family: 'Montserrat', sans-serif; margin-left: 10px; color: white; margin-right: 10px; height:45px; float:left; position:relative; z-index:1;  border-radius:5px; overflow:hidden; border:solid 1px #7FDBFF; margin-top: 10px; margin-bottom: 10px; display: none; visibility:hidden;">
                            ...
                            <div id="progress-wrp0" style="color: white;" class="progress-wrp">
                        ...
                        <div id="contenedorcaracteristicas" style="width: 100%; float:left;">
                            ...
                            <div id="contenedorfechainicio" style="width: calc(50% - 10px); float: left; padding: 5px;">
                                ...
                                <input id="fechainicio0" name="fechainicio0" align="center" style="width:  calc(100% - 10px); line-height: 20px; padding-left: 5px;" type="text" class="hasDatepicker">
                            ...
                            <div id="contenedorfechafinal" style="width: calc(50% - 10px); float: left; padding: 5px;">
                                ...
                                <input id="fechafinal0" name="fechafinal0" align="center" style="width: calc(100% - 10px); line-height: 20px; padding-left: 5px;" type="text" class="hasDatepicker">
                    ...
                    <div id="contenedorselects" style="width: 100%;float: left; margin-top:20px">

                        <div id="tituloagregarmateria" style="width:100%; float:left;"> Materias </div>

                        <select id="materias" name="materias" style="font-size: 12px;width: calc(50% - 20px);margin: 10px 0;box-sizing: border-box;padding: 10px 10px;border: 1px solid #e8e1e1;background-color: whitesmoke;color: gray;float: left;/* margin-top: 30px; */">Materias


                            <option value="1">Agregar materias</option>
                            ...
                            ?><option value="<?php echo $idmateria; ?>"><?php echo $materia; ?></option><?php
                            } while ($row_Recordset14 = mysql_fetch_assoc($Recordset14)); ?>
 

                        </select>

                        <div id="lineaunion" style="width: 30px;border: solid lightgray;height: 0px;float: left;margin-top: 26px;">

                        </div>

                        <select id="maestros " name="maestros" style="font-size: 12px;width: calc(50% - 15px);margin: 10px 0;box-sizing: border-box;padding: 10px 10px;border: 1px solid #e8e1e1;background-color: whitesmoke;color: gray;float: right;">Maestros


                            <option value="1">Agregar maestro</option>
                    ...
                    </div><!--Fin de contenedor selects-->

                   

                    
                    <div id="contenedorbotones" align="center" style="float: left; width: 100%; margin-top: 50px; margin-bottom: 20px;">
                            <button id="botonagregar" type="button" onclick="javascript:agregarcurso('');" style="font-family: montserrat;border: solid #218632 2px;background: #067c259e;float: left; border-radius: 5px;width: 100px;font-size: 12px;cursor: pointer;color: white;padding-top: 10px;padding-bottom: 10px;">Agregar materia</button>
                            <button id="botoneliminar" type="button" onclick="javascript:eliminarcurso('');" style="font-family: montserrat;border: solid #ed0000 2px;background: #f50000b5;float: left; border-radius: 5px;width: 100px;margin-left: 50px;cursor: pointer;font-size: 12px;color: white;padding-top: 10px;padding-bottom: 10px;">Eliminar materia</button>
                    </div>

                <div id="contenedorgrupos" style="margin-top: 30px;width:100%;float:left; margin-bottom:10px; "> 
                    
                    <div id="grupos" style="text-align: center;width:100%;float:left;margin-bottom: 15px;">    Grupos </div>

                    <div id="grupo1" style="background: #164574e0;border:solid #1d4267;height:60px;float:left;width:15%;border-radius:5px;">

                    <input type="text" style="text-align: center;width: 42%;margin-top: 20px;margin-left: 47px;margin-right: 15px;float: left;">                    
                    
                    </div>

                    <div id="grupo2" style="background: #164574e0; border:solid #1d4267; margin-left: 15px; height:60px; float:left;width:15%; border-radius:5px;">

                    <input type="text" style="text-align: center;width: 42%;margin-top: 20px;margin-left: 47px;margin-right: 15px;float: left;">                    
                    
                    </div>

                    <div id="grupo3" style="background: #164574e0; border:solid #1d4267; margin-left: 15px; height:60px; float:left;width:15%; border-radius:5px;">

                    <input type="text" style="text-align: center;width: 42%;margin-top: 20px;margin-left: 47px;margin-right: 15px;float: left;">                    
                    
                    </div>

                    <div id="grupo4" style="background: #164574e0; border:solid #1d4267; margin-left: 15px; height:60px; float:left;width:15%; border-radius:5px;">

                    <input type="text" style="text-align: center;width: 42%;margin-top: 20px;margin-left: 47px;margin-right: 15px;float: left;">                    
                    
                    </div>

                    <div id="grupo5" style="background: #164574e0; border:solid #1d4267; margin-left: 15px; height:60px; float:left;width:15%; border-radius:5px;">

                    <input type="text" style="text-align: center;width: 42%;margin-top: 20px;margin-left: 47px;margin-right: 15px;float: left;">                    
                    
                    </div>

                    <div id="grupo6" style="background: #164574e0; border:solid #1d4267; margin-left: 15px; height:60px; float:left;width:15%; border-radius:5px;">

                    <input type="text" style="text-align: center;width: 42%;margin-top: 20px;margin-left: 47px;margin-right: 15px;float: left;">                    
                    
                    </div>
            
               </div><!--Fin conteneor grupos-->
           

                    <div id="contenedorboton" align="center" style="float: left; width: 100%; margin-top: 50px; margin-top: 20px;">
                        <button type="button" onclick="guardarform('');" style=" padding: 5px 15px; background-color: #1d4267; font-size: 14px; border-color: #7FDBFF; color: #7FDBFF; cursor: pointer;">Guardar cambios</button>
                    </div>
                ...
                <input type="hidden" id="cmaterias" name="cmaterias" value="1">
...
?>
            ...
            <div id="curso<?php $id; ?>" style="overflow: hidden;width:calc(100% - 20px);height: 155px;background-color:#FFF;display:inline-block; border-bottom:1px solid lightgray;padding-bottom: 20px;padding-top: 20px; margin-left: 10px; margin-right: 10px;">
                ...
                <a href="vercurso.php?id=<?php echo $id; ?>"><div id="imagen<?php $id; ?>" style="width: 20%;height: 150px;float:left;padding-left: 10px;padding-right: 10px;background-image:url('../images/<?php echo $imagen; ?>');background-size: cover;background-position: center;background-repeat: no-repeat;">
                </div></a>

                <a href="vercurso.php?id=<?php echo $id; ?>"> <div id="title" style="padding-right: 5%;padding-left: 5%;text-align: left;letter-spacing: 1.5px;width: 65%;line-height:20px;float:left;margin-top:20px;color:#6A6969;font-size: 18px;font-weight:bold;"><?php echo $nombre; ?></div></a>

                <a href="vercurso.php?id=<?php echo $id; ?>"><div id="descripcion" align="center" style="text-align: left;width: 65%;padding-left:5%;height: 55px;overflow: hidden;padding-right:5%;float:left;margin-top:20px;color:#6A6969;font-size: 14px;"><?php echo $descripcion; ?></div></a>
